...
var getKeyFromCode = function (code) {
    var keys = uiohook_napi_1.UiohookKey;
    return Object.keys(uiohook_napi_1.UiohookKey).find(function (key) { return keys[key] === code; });
};
var Hotkeys = /** @class */ (function () {
    function Hotkeys() {
        ...
        this.register = function (keys, action) {
            if (!keys)
                return console.log("Error reading hotkeys: ".concat(action));
        ...
        this.run = function (debug) {
            ...
            uiohook_napi_1.uIOhook.on('keydown', function (event) {
                ...
                if (!_this.keys_pressed.includes(key)) {
                    ...
                    if (debug)
                        console.log("Pressed: ".concat(getKeyFromCode(key)));
